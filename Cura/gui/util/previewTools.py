...
class toolRotate(object):
	...
	def OnDrag(self, p0, p1):
		if not self.dragStartAngle == None:
			cursorX0, cursorY0, cursorZ0, cursorYZ, cursorXZ, cursorXY = self._ProjectToPlanes(p0, p1)
...
class toolScale(object):
	...
	def OnDrag(self, p0, p1):
		...
		if wx.GetKeyState(wx.WXK_SHIFT):
			...
			if self.node == 1 or not wx.GetKeyState(wx.WXK_CONTROL):
				matrixScale = (scaleX + scaleY + scaleZ) / 3.0
			elif self.node == 2:
				matrixScale = scaleX
			elif self.node == 3:
				matrixScale = scaleY
	...
	def OnDraw(self):
		...
		if self.node == 1:
			...
			axisToScale = "Z"
		try:
			self.parent.updateScaleForm(size[0], size[1], size[2], scaleX, scaleY, scaleZ, axisToScale)
		except:
			pass
