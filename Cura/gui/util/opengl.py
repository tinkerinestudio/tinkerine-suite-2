...
class GLVBO(GLReferenceCounter):
	def __init__(self, vertexArray, normalArray = None):
		...
		if not bool(glGenBuffers):
			...
			if self._hasNormals:
				glBufferData(GL_ARRAY_BUFFER, numpy.concatenate((self._asdf, normalArray), 1), GL_STATIC_DRAW)
			else:
...
def loadGLTexture2(pBits, textWidth, textHeight):
	tex = glGenTextures(1)
	glBindTexture(GL_TEXTURE_2D, tex)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR)
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, textWidth, textHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, pBits)
	return tex

def loadGLTexture3(tooltipText):
	try:
		tex = tooltipText._texture
		textWidth = tooltipText._textWidth
		textHeight = tooltipText._textHeight
		pBits = tooltipText.makeText()
		if tex is None:
			tex = glGenTextures(1)
		glBindTexture(GL_TEXTURE_2D, tex)
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
