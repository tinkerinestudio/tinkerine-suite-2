...
class mainWindow(wx.Frame):
	def __init__(self):
		...
		self.SetDropTarget(dropTarget.FileDropTarget(self.OnDropFiles, meshLoader.loadSupportedDragInExtensions()))
		...
		# if version.isDevVersion():
		i = expertMenu.Append(-1, _('Open Debug Settings...'))
		self.normalModeOnlyItems.append(i)
		...
		i = expertMenu.Append(-1, _('Update Printer Firmware'))
		...
		machineMenu = wx.Menu()
		i = machineMenu.AppendRadioItem(1, _('DittoPro'))
		self.Bind(wx.EVT_MENU, self.changeToDittoPro, i)
		i = machineMenu.AppendRadioItem(2, _('Ditto+'))
		self.Bind(wx.EVT_MENU, self.changeToDitto, i)
		i = machineMenu.AppendRadioItem(3, _('Litto'))
		self.Bind(wx.EVT_MENU, self.changeToLitto, i)


		if profile.getPreferenceFloat('machine_width') == 130: #litto width
			machineMenu.Check(3, True)
		elif profile.getPreferenceFloat('machine_width') == 210: #ditto width
			machineMenu.Check(2, True)
		else:
			machineMenu.Check(1, True)

		self.menubar.Append(machineMenu, _('Printer'))
		...
		i = aboutMenu.Append(-1, _('About Tinkerine Suite'))
		self.Bind(wx.EVT_MENU, self.OnAbout, i)
		self.menubar.Append(aboutMenu, _('About'))


		helpMenu = wx.Menu()
		i = helpMenu.Append(-1, _('First Run Tour'))
		self.Bind(wx.EVT_MENU, self.OnFirstRunTour, i)

		i = helpMenu.Append(-1, _('Keyboard Shortcuts'))
		self.Bind(wx.EVT_MENU, self.OnKeyboardShortcuts, i)
		...
		if profile.getPreference('show_advanced') == 'True':
			expertMenu.Check(self.showAdvancedSettings.GetId(), True)

		self.updateSliceMode()

	def toggleAdvanced(self, e):

		if profile.getPreference('show_advanced') == 'True':
			profile.putPreference('show_advanced', 'False')
		else:
			profile.putPreference('show_advanced', 'True')
		self.scene.updateAdvancedModeButtonPositions()
		self.scene._drawGui()
		self.scene._OnSize(e)

	def toggleUniformScaling(self, e):
		if self.scene.rotationLock == True:
			self.scene.rotationLock = False
		else:
	...
	def changeToLitto(self, e):
		self.scene.changeToLitto(e)

	def changeToDitto(self, e):
		self.scene.changeToDitto(e)

	def changeToDittoPro(self, e):
		self.scene.changeToDittoPro(e)
	...
	def OnDropFiles(self, files):
		...
		if gcodeFilename is not None:
			...
			self.scene.OnSliceDone(gcodeFilename, False)
			self.scene.OnViewChange()
		else:
			if firmwareFilename is not None:
				firmwareInstall.InstallFirmware(firmwareFilename)
				return
			if self.scene.getModelView() == 4:
				self.scene.setModelView(1)
				self.scene.OnViewChange()
	...
	def OnCustomFirmware(self, e):
		...
		if dlg.ShowModal() == wx.ID_OK:
			...
			if not(os.path.exists(filename)):
				return
			if len(machineCom.serialList(True)) > 1:
				pass #choice goes here
	...
	def OnAbout(self, e):
		...
		info.SetDescription(_('\tPlating, Preparing and Slicing for Fused Filament Fabrication 3D printing.'))
		info.SetWebSite(_('http://www.tinkerine.com'))
		info.SetCopyright(_('Copyright (C) Tinkerine 2016'))
		info.SetLicence(_("""
...
"""))
	...
	def OnFirstRunTour(self, e):
		self.SetSize((1024,768))
		self.scene.properScaledWidth = self.scene._parent.GetSize()[0]
		self.scene.tourCount = 0
		profile.putPreference('first_run_tour_done', 'False')

	def OnKeyboardShortcuts(self, e):
		self.scene.showKeyboardShortcuts = True
		self.scene._drawGui()
