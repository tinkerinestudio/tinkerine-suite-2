...
def getSliceStatsReportLocation():
	slicePathFile = os.path.abspath(os.path.join(os.path.split(os.path.abspath(__file__))[0], "../stats_location"))
	if os.path.exists(slicePathFile):
		f = open(slicePathFile, "r")
		version = f.readline()
		f.close()
		return version.strip()
	else:
