...
class gcode(object):
	...
	def _load(self, gcodeFile):
		...
		for line in gcodeFile:
			...
			if ';' in line:
				...
				if comment.startswith('LAYER:'):
					...
					currentLayerNumber = comment.split(":")[-1]
			...
			if G is not None:
				if G == 0 or G == 1:	#Move
					...
					if pathType == 'SUPPORT' and moveType == 'extrude':
						pass
						#print currentPath['points']
				...
				if M is not None:
					if M == 0:	#Message with possible wait (ignored)
						...
						pass
					elif M == 25: #A pause to change filament
						self.pauses.append(int(currentLayerNumber)+1)
		...
		timeMultiplier = translate(totalMoveTimeMinute, 15, 240, 1.1, 1.20)
