...
class EngineResult(object):
	...
	def applyReplaceTags(self, longSearch = False):
		if longSearch == False:
			self._gcodeData.replaceAtStart(self._replaceInfo)
		else:
			self._gcodeData.replaceAtStart(self._replaceInfo, True)
	...
	def submitInfoOnline(self):
		...
		if version.isDevVersion():
			return
		data = json.dumps({
			'resolution': profile.getProfileSetting('layer_height'),
			'wall_thickness': profile.getProfileSetting('wall_thickness'),
			'fill_density': profile.getProfileSetting('fill_density'),
			'print_speed': profile.getProfileSetting('print_speed'),
			'print_temperature': profile.getProfileSetting('print_temperature'),
			'filament_diameter': profile.getProfileSetting('filament_diameter'),
			...
			'print_time': (1+self._printTimeSeconds)/60,
			'filament_amount': int(self.getFilamentWeight()*1000),

			'machine_width': int(profile.getPreferenceFloat('machine_width')),
			'brand': version.getBrand(),
		})
		#print data
		try:
			url = version.getSliceStatsReportLocation()
			req = urllib2.Request(url, data, {'Content-Type': 'application/json'})
			f = urllib2.urlopen(req)
			f.read()
			f.close()
		except:
			#print "could not report stats"
			if version.isDevVersion():
				import traceback
				traceback.print_exc()
...
class Engine(object):
	...
	def _runEngine(self, scene, overrides, old_thread):
		...
		if order is None:
			...
			objMax = None
			for obj in scene.objects():
				obj.applyMatrix(numpy.matrix([[1,0,0],[0,1,0],[0,0,1]], numpy.float64))
				if scene.checkPlatform(obj):
		...
		try:
			...
			if returnCode == 0:
				...
				firstG0Line = ''
				block0 = self._result._gcodeData._list[0].getvalue()[0:2048]
				lines = block0.split('\n')
				for line in lines:
					if line.startswith("G0"):
						firstG0Line = line
						break
				params = firstG0Line.split(' ')
				x = 0
				y = 0
				f = 0
				for param in params:
				...
				startingExtrusion = distance*0.03
				startSpeed = float(f)/3
			...
			# 	block0 = block0.replace(key, str(value), 1)
			# self._list[0].write(block0)

			# return _('%0.2f meter %0.0f gram') % (float(self._filamentMM[e]) / 1000.0, self.getFilamentWeight(e) * 1000.0)

				self._result.addReplaceTag('LAYER:1', 'LAYER:1\r\nM104 S' + str(profile.getProfileSettingFloat('print_temperature')))
				self._result.applyReplaceTags(True)
				self._result.addReplaceTag(firstG0Line, "G1 F%f X%0.2f Y%0.2f E%0.2f\r\n" % (startSpeed, x-10, y-10, startingExtrusion) + firstG0Line + "\r\nG92 E0\r\n")
				# self._result.addReplaceTag('G0 F%s' % f, 'G92 E-%0.2f\r\nG0 F%f E0' % (startingExtrusion, halfStartSpeed))
				...
				self._submitDataThread = threading.Thread(target=self._result.submitInfoOnline())
	...
	def _watchStderr(self, stderr):
		...
		while len(line) > 0:
			...
			if line.startswith('Progress:'):
				...
				timeMultiplier = translate(seconds, 15*60, 240*60, 1.1, 1.30)
				self._result._printTimeSeconds = seconds*timeMultiplier
	...
	def _engineSettings(self, extruderCount):
		settings = {
			...
			'supportExtrusionWidth': int(profile.getProfileSettingFloat('support_extrusion_width') * 1000),
			# 'layer0extrusionWidth': 400,
			'insetCount': int(profile.calculateLineCount()),
			'downSkinCount': int(profile.getProfileSettingFloat('bottom_surface_thickness_layers')),
			'upSkinCount':  int(profile.getProfileSettingFloat('top_surface_thickness_layers')),
			'infillOverlap': int(profile.getProfileSettingFloat('fill_overlap')),
			'perimeterBeforeInfill': 1 if profile.getProfileSetting('perimeter_before_infill').lower() == 'true' else 0,
			...
			'inset0Speed': int(profile.getProfileSettingFloat('print_speed')* float(profile.getProfileSettingFloat('perimeter_speed_ratio')/100)),
			'insetXSpeed': int(profile.getProfileSettingFloat('print_speed')* float(profile.getProfileSettingFloat('perimeter_speed_ratio')/100)),
			# 'Speed': int(profile.getProfileSettingFloat('solidarea_speed')) if int(profile.getProfileSettingFloat('solidarea_speed')) > 0 else int(profile.getProfileSettingFloat('print_speed')),
			'skinSpeed': int(profile.getProfileSettingFloat('print_speed') * float(profile.getProfileSettingFloat('skin_speed_ratio')/100)),
			'moveSpeed': int(profile.getProfileSettingFloat('travel_speed')),
			'fanSpeedMin': int(profile.getProfileSettingFloat('fan_speed_max')) if profile.getProfileSetting('fan_enabled') == 'True' else 0,
			...
			'extraSupportAmount': int(1000 * profile.getProfileSettingFloat('extra_support_amount')),
			...
			'retractionAmount': int(profile.getProfileSettingFloat('retraction_amount') * 1000) if profile.getProfileSetting('retraction_enable').lower() == 'true' else 0,
			...
			'retractionAmountExtruderSwitch': int(profile.getProfileSettingFloat('retraction_min_travel') * 1000),
			...
			'preSwitchExtruderCode': profile.getAlterationFileContents('preSwitchExtruder.gcode'),
			'postSwitchExtruderCode': profile.getAlterationFileContents('postSwitchExtruder.gcode'),
			'replace.csv': profile.getAlterationFileContents('replace.csv'),
		...
		settings['fanFullOnLayerNr'] = int(profile.getProfileSettingFloat('fan_layer'))
		...
		if profile.getProfileSetting('brim_enable') == 'Enabled':
			settings['skirtDistance'] = 0.2
		...
		if profile.getPreferenceFloat('gcode_flavor') == 'UltiGCode':
			settings['gcodeFlavor'] = 1
		elif profile.getPreferenceFloat('gcode_flavor') == 'MakerBot':
			settings['gcodeFlavor'] = 2
		elif profile.getPreferenceFloat('gcode_flavor') == 'BFB':
			settings['gcodeFlavor'] = 3
		elif profile.getPreferenceFloat('gcode_flavor') == 'Mach3/LinuxCNC':
			settings['gcodeFlavor'] = 4
		elif profile.getPreferenceFloat('gcode_flavor') == 'RepRap (Volumetric)':
